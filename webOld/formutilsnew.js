...
function checkDateTime(did, tid, required) {
    ...
    if (dValue  && !checkDate(did)) return false;
    if (!dValue && required)        return false;
     
    tValue = fieldHasValue(tid, required);
    
    if (tValue  && !checkTime(tid)) return false;
    if (!tValue && required)        return false;
...
function getNameValue(id) {
    var elm = getElementId(id);
    
    return elm.id + "=" + elm.type + "," + getValue(id);
}
    ...
    function addColumn() {
        ...
        while (json.next(":,}")) {
            ...
            if (json.type === "}") {
                ...
                if (optional === true)
                    cell.addAttrubute("class", "optional");
    ...
    while (json.next("[],")) {
        ...
        while (json.next(",]")) {
            ...
            if (!json.quoted) {
                ...
                if (typeof nullNumberToSpace !== 'undefined' && nullNumberToSpace && value === 'null')
                    value = '';
            }
...
//                    cell.setAttribute("style", 'width:' + getWidth(size, 'em'));
                ...
                if (cols.getColumn(i).optional)
                    cell.setAttribute("class", "optional");
...
function discardFieldInput (msg) {
    ...
    if (msg !== undefined) displayAlert('', msg);
...
function setLabel(name, caption) {
    getElement(name).innerHTML = caption;
}
